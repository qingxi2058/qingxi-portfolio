...
export async function onRequest(context) {
  ...
  if (request.method !== 'POST') {
    return json(405, { success: false, error: '只支持 POST 请求', code: 'METHOD_NOT_ALLOWED' }, corsHeaders);
  }
  ...
  try {
    ...
    return json(400, { success: false, error: '请求体不是合法 JSON', code: 'INVALID_JSON' }, corsHeaders);
  }
  ...
  const userInput = [
    `剧本类型：${requestBody.genre || 'modern'}`,
    `预计时长：${requestBody.duration || '3min'}`,
    `创意程度：${requestBody.creativity || 5}/10`,
    `关键词：${requestBody.keywords || '时间胶囊、咖啡馆、反转、真相'}`,
  ].join('\n');
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return json(response.status, { success: false, error: message, code: 'MODEL_REQUEST_FAILED' }, corsHeaders);
    }

    const content = readContent(payload?.choices?.[0]?.message?.content);
    const parsed = parseJsonText(content);
    const normalized = normalizeProject(parsed, requestBody);

    return json(200, { success: true, data: normalized }, corsHeaders);
  } catch (error) {
    return json(500, {
      success: false,
